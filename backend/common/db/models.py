...
class KnowledgeBase(Base):
    ...
    icon_name = Column(String(100), nullable=True, comment='图标名称')
    collection_id = Column(Integer, nullable=False, comment='绑定的VDBCollection ID')  # 新增字段，必须绑定
...
class VDB(Base):
    ...
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    shares = relationship("VDBShare", primaryjoin="VDB.id == foreign(VDBShare.vdb_id)")

class VDBCollection(Base):
    __tablename__ = "vdb_collection"
