...
from common.schemas.oss_connection import OSSConnectionCreate, OSSConnectionUpdate, OSSConnectionOut, OSSConnectionTest, ShareOSSConnectionIn
from webapi.services.oss_connection_service import (
...
from common.db.models import OSSConnection, OSSConnectionShare, Team
...
def list(team_id: int = None, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    ...
    result = []
    for item in data:
        d = OSSConnectionOut.model_validate(item, from_attributes=True).model_dump()
...
def get_oss_connection_detail(id: int, db: Session = Depends(get_db)):
    ...
    data = OSSConnectionOut.model_validate(conn, from_attributes=True).model_dump()
