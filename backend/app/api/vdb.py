...
from app.db.models import Team, VectorDBConfig, Collection, User, UserTeam, VDBShare  # 新增 VDBShare
...
def list_vdbs(team_id: Optional[int] = Query(None), db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    # 拥有的
    owned = db.query(VectorDBConfig).filter(VectorDBConfig.team_id == team_id).all()
    ...
    for share in db.query(VDBShare).filter(VDBShare.team_id == team_id, VDBShare.status == 'active').all():
        vdb = db.query(VectorDBConfig).filter(VectorDBConfig.id == share.vdb_id).first()
    ...
    for share in db.query(VDBShare).filter(VDBShare.team_id == team_id, VDBShare.status == 'revoked').all():
        ...
        if vdb and vdb.id not in owned_ids and vdb.id not in shared_ids:
            # 只展示有 collection 的
            has_collection = db.query(Collection).filter(Collection.vdb_id == vdb.id, Collection.team_id == team_id).first()
            if has_collection:
...
def update_vdb(id: int, config_in: VectorDBConfigIn, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    config = db.query(VectorDBConfig).filter_by(id=id).first()
    if not config:
        return BaseResponse(code=404, message="未找到该向量数据库实例")
...
def share_vdb(id: int, share_in: ShareVDBIn, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    vdb = db.query(VectorDBConfig).filter_by(id=id).first()
    if not vdb:
        return BaseResponse(code=404, message="未找到该向量数据库实例")
